...
const navigationRoutes: NavigationMenuRoutes = [
  {
    ...
    label: "Home",
    icon: <Home />,
    children: [{ path: "counter", label: "Counter", icon: <Calculate /> }],
  },
];
...
const NavigationList = styled(OpenableList, {
  shouldForwardProp: (prop) => prop != "open",
})<OpenableListProps>(({ theme, open }) => ({
  height: "100%",
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          {children?.map((nestedRoute) => (
            <ExpandedList
              key={[joinPath, nestedRoute.path].join("/")}
              route={nestedRoute}
  ...
  const menuItem = (
    ...
    >
      ...
      {isOpen && children?.length && (
        <IconButton onClick={(e) => onToggleExpand(e)}>
...
export const NavigationMenu: Component = () => {
  ...
  return (
    <Grid xs="auto">
      <NavigationList dense disablePadding open={open}>
